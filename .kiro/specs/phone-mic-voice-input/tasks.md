...
- [ ] 7. Input_Injector（跨平台键盘注入）
  ...
  - [ ] 7.1 定义 Input_Injector trait 与平台桥接骨架
  ...
  - [ ] 7.6 实现 Windows 后端（SendInput + KEYEVENTF_UNICODE）
  ...
  - [ ] 7.7 实现 macOS 后端（CGEvent + AX）与权限引导接入
  ...
  - [ ] 7.8 实现 Linux 后端（X11 XTEST + Wayland 兜底说明）
  ...
  - [ ] 7.11 实现 INJECT_NO_FOCUS_TARGET 与 inject.error 上报
  ...
  - [ ] 7.13 编写 Input_Injector 平台单元 / 集成测试
...
- [ ] 8. ASR Bridge（Server_ASR）
  ...
  - [ ] 8.2 实现 ASR 引擎决策函数 `pick_asr_engine`
...
- [ ] 9. Mobile_Web_Client（Vue 3 SPA）
  ...
  - [ ] 9.3 实现 ConnectionService（WebSocket 客户端）
  ...
  - [ ] 9.4 实现连接状态文本映射 `statusLabel(s, lang)`
  ...
  - [ ] 9.6 实现录音状态机（press / toggle）与录音视觉指示
  ...
  - [ ] 9.8 实现 Browser_ASR 适配（Web Speech API）与 interim 实时显示
...
- [ ] 10. 桌面端 UI（Tauri + Vue / 原生 webview）
  ...
  - [ ] 10.2 实现 Pairing_Code 与连接 URL 面板
...
- [ ] 13. 错误处理、可观测性与日志
  ...
  - [ ] 13.3 接入 inject.error / asr.error 上报通道
...
- [ ] **功能完备**：任务 1–14 中所有非 `*` 任务全部完成
- [ ] **可选测试补齐**：所有 `*` 任务（属性 / 集成测试）全部完成且通过
- [ ] **34 条 Property 全部存在并通过**：CI 中 `cargo test --features property-tests` 与 `pnpm test -- --run` 均显示 34 条 property 测试全部 PASS（与 design §7 一一对应）
- [ ] **跨平台冒烟通过**：任务 12.3 在 Windows 10/11、macOS 12+、Ubuntu 20.04+ 三个目标 OS 上 PASS
- [ ] **启动 / 关闭时延达标**：任务 10.8 / 5.1 验证启动 ≤ 5 秒、关闭 ≤ 3 秒（Requirements 1.2, 2.7）
- [ ] **安全验证通过**：HTTPS 模式机密信息不泄漏（Property 23）、子网过滤（Property 21）、限流（Property 19）属性测试 PASS；Session_Token 落盘加密（design §5.4）已经过验证
- [ ] **可观测性完整**：日志滚动（Property 32）属性测试 PASS；inject.error / asr.error 通道已验证（Property 33）；诊断包导出可用
- [ ] **i18n 完整**：zh-CN 与 en-US 字典 key 集合相等（任务 3.24 / 9.25）；切换即时生效（Property 25）；UI / ASR 语言独立（Property 26）
- [ ] **CI 全绿**：`cargo fmt --check`、`cargo clippy -D warnings`、`cargo test`、`pnpm lint`、`pnpm test`、`pnpm build`、跨平台构建产物上传成功
- [ ] **签名与版本一致**：任务 14.2 校验通过；release tag 与 `/api/health` 返回的 `version` 一致
